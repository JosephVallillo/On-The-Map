...
class UdacityClient {
    ...
    private func makeRequestForUdacity(url url: NSURL, method: HTTPMethod, body: [String:AnyObject]? = nil, headers: [String:String]? = nil, responseHandler: (jsonAsDictionary: [String:AnyObject]?, error: NSError?) -> Void) {
        ...
        apiSession.makeRequestAtURL(url: url, method: method, headers: allHeaders, body: body) { (data, error) in
            if let data = data {
    ...
    func loginWithUsername(username: String, password: String, facebookToken: String? = nil, completionHandler: (userKey: String?, error: NSError?) -> Void) {
        ...
        makeRequestForUdacity(url: loginURL, method: .POST, body: loginBody) { (jsonAsDictionary, error) -> Void in
    ...
    func logout(completionHandler: (success: Bool, error: NSError?) -> Void) {
        ...
        makeRequestForUdacity(url: logoutURL, method: .DELETE, headers: logoutHeaders) { (jsonAsDictionary, error) -> Void in
            guard error == nil else {
    ...
    func studentWithUserKey(userKey: String, completionHandler: (student: Student?, error: NSError?) -> Void) {
        let userURL = apiSession.urlForMethod(Methods.Users, withPathExtension: "/\(userKey)")
        
        makeRequestForUdacity(url: userURL, method: .GET) { (jsonAsDictionary, error) -> Void in
            guard error == nil else {
